...
/**
 * Two sources behind a single resolver, to show that it can be done.
 *
 * Lucide (ISC) hands back Vue components: no network traffic and tree-shakeable.
 * Lucide deliberately ships no brand marks, so those arrive here as raw SVG strings
 * from Simple Icons (CC0). A third option is to return a URL -- PickyLibrary then
 * fetches it and inlines it so currentColor keeps working.
 */
...
// Simple Icons, CC0. No stroke and no fill of their own, so PickyLibrary adds
// fill="currentColor" here while leaving Lucide's stroke attributes alone.
